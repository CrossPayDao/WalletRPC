...
- **回归测试强制（研发测试原则）**：凡修复任何 bug（功能/逻辑/性能/体验/兼容性/适配性），必须同步补充至少一条自动化回归测试以锁定该问题，避免下次回归再次出现。
  - 优先选择单元测试；若问题跨组件/跨页面，则使用 UI 测试或 E2E。
...
- 实现阶段的测试要求：以“最小可用回归”为主。
  - 修 bug 时应优先同步补对应回归测试（遵循“回归测试强制”原则）。
  - 对新功能可先补核心单元测试骨架，但不要求在本阶段完成完整自动化覆盖。
...
0. **测试补齐主阶段（Test Completion Stage）**
   - 自动化测试（单元测试 / UI 测试 / E2E）应在本阶段集中补齐并达标。
...
1. **代码与文档一致性对齐**
   ...
   - 若不一致：必须二选一并记录
     ...
     - 提出不一致点并修复代码使其符合文档。
2. **补齐自动化测试**
   - 若验证阶段仍存在缺口，在本阶段补齐剩余测试覆盖。
   - 本阶段定位为“补漏与收口”，不应新增大规模测试范围；若存在系统性缺口，应回退至验证阶段处理。
3. **性能与体验问题修复**
