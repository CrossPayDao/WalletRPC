    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans flex flex-col animate-in fade-in duration-700">
      ...
      <header className="bg-white/80 border-b border-slate-200 sticky top-0 z-40 px-4 md:px-8 h-16 flex items-center justify-between shadow-sm backdrop-blur-md">
         ...
         <div className="flex items-center space-x-4">
             ...
             <div className="hidden lg:flex items-center mr-4 border-r border-slate-100 pr-6">
                ...
                <span className="font-black tracking-tighter italic text-slate-900 flex items-center">
                  ...
                  <span className="ml-2 bg-slate-900 text-white text-[8px] px-1.5 py-0.5 rounded uppercase tracking-widest font-black">BETA</span>
                </span>
             </div>
             ...
             <div className="relative">
                <button 
                  ...
                  `}
                >
                  <div className={`relative w-8 h-8 rounded-lg flex items-center justify-center shadow-sm ${activeAccountType === 'EOA' ? 'bg-[#0062ff] text-white' : 'bg-[#00d4ff] text-white'}`}>
                  ...
                  <div className="text-left hidden md:block">
                     <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">
                        {activeAccountType === 'EOA' ? 'Active Key' : 'Node Master'}
                     </div>
                     <div className="text-sm font-black text-slate-900 truncate max-w-[150px] uppercase italic tracking-tight">
                        {activeAccountType === 'EOA' ? (activeChain.chainType === 'TRON' ? 'Tron_Node' : 'Master_Key') : `Safe_${activeSafeAddress?.slice(0,4)}`}
                     </div>
                  </div>
                   ...
                   <div className="absolute top-full left-0 mt-3 w-72 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-tech-in z-50">
                      <div className="p-2 border-b border-slate-100">
                         <button onClick={() => { setActiveAccountType('EOA'); setIsMenuOpen(false); setView('dashboard'); }} className="w-full text-left p-3 hover:bg-slate-50 rounded-xl flex items-center transition-colors group">
                            ...
                            <div>
                               <div className="text-sm font-black text-slate-900 uppercase italic">Master Key</div>
                               <div className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Local EOA Context</div>
                            </div>
                         </button>
                      </div>
                      <div className="p-2 max-h-[300px] overflow-y-auto">
                         <p className="text-[10px] font-black text-slate-400 px-3 py-2 uppercase tracking-[0.2em]">Verified Safes</p>
                         {trackedSafes.filter(s => s.chainId === activeChainId).map(s => (
                            <div key={s.address} className="flex justify-between items-center group mb-1">
                               <button 
                                  ...
                                  className="flex-1 text-left p-2.5 text-xs flex items-center rounded-xl hover:bg-blue-50 hover:text-[#0062ff] transition-colors"
                               >
                                  <div className="w-1.5 h-1.5 bg-[#0062ff] rounded-full mr-3 shadow-[0_0_8px_rgba(0,98,255,0.5)]"></div>
                                  <span className="font-mono font-medium truncate uppercase">{s.name}</span>
                                </button>
                         ...
                         {trackedSafes.filter(s => s.chainId === activeChainId).length === 0 && (
                           <div className="px-3 py-4 text-[10px] text-slate-300 text-center italic border border-dashed border-slate-100 rounded-xl mb-2 font-bold uppercase tracking-widest">Empty Vault</div>
                         )}
                         <div className="grid grid-cols-2 gap-2 mt-2 pt-2 border-t border-slate-100">
                            <button onClick={() => { setView('create_safe'); setIsMenuOpen(false); }} className="py-2 text-[10px] bg-[#0062ff] text-white rounded-lg font-black uppercase hover:bg-[#0052d9] transition-colors">DEPLO_NEW</button>
                            <button onClick={() => { setView('add_safe'); setIsMenuOpen(false); }} className="py-2 text-[10px] bg-white border border-slate-200 text-slate-400 rounded-lg font-black uppercase hover:text-slate-900 hover:border-slate-300 transition-colors">IMPORT</button>
                         </div>
                      </div>
                   </div>
                )}
         ...
         <div className="flex items-center gap-2">
            <button onClick={() => setIsChainModalOpen(true)} className="p-2.5 hover:bg-slate-100 rounded-full text-slate-400 hover:text-[#0062ff] transition-colors" title="Settings">
            ...
            <button 
               ...
               className="flex items-center space-x-2 px-3 py-2 hover:bg-red-50 rounded-xl text-slate-400 hover:text-red-600 transition-colors"
               title="Kill Session"
            >
               <LogOut className="w-4 h-4"/>
               <span className="text-[10px] font-black hidden md:inline uppercase tracking-widest">KILL_SIG</span>
            </button>
         </div>
      </header>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
               {view === 'dashboard' && (
                  <>
                    ...
                    />
                    <div className="mt-12 mb-6 text-center opacity-20">
               ...
               {view === 'send' && (
                  <SendForm 
                    activeChain={activeChain}
                    tokens={activeChainTokens}
                    balances={{ ...tokenBalances, NATIVE: balance }}
                    activeAccountType={activeAccountType}
