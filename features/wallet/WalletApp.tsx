                      ...
                      <div className="p-2 max-h-[300px] overflow-y-auto">
                         ...
                         {trackedSafes.filter(s_safe => s_safe.chainId === activeChainId).map(s_safe => (
                            <div key={s_safe.address} className="flex justify-between items-center group mb-1">
                               ...
                               <button onClick={(e) => { e.stopPropagation(); setTrackedSafes(prev => prev.filter((x) => !(x.address.toLowerCase() === s_safe.address.toLowerCase() && x.chainId === activeChainId))); if (activeSafeAddress?.toLowerCase() === s_safe.address.toLowerCase()) { setActiveAccountType('EOA'); setActiveSafeAddress(null); setView('dashboard'); } }} className="p-2 text-slate-300 hover:text-red-500 transition-colors"><Trash2 className="w-3.5 h-3.5"/></button>
                            </div>
                         ))}
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 relative scroll-smooth">
         <div className="max-w-5xl mx-auto relative z-10">
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
               ...
               {view === 'send' && <SendForm activeChain={activeChain} tokens={activeChainTokens} balances={{ ...tokenBalances, NATIVE: balance }} activeAccountType={activeAccountType} recommendedNonce={currentNonce} onSend={handleSendSubmit} onBack={() => setView('dashboard')} onRefresh={handleRefreshData} isLoading={isLoading} transactions={transactions} />}
